...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options && options.shareCode) {
      patch.shareCode = options.shareCode
      patch.isRegister = true
      // 追踪分享信息
      this.traceShareCode(options.shareCode)
  ...
  traceShareCode: function (shareCode) {
    var self = this
    api.cardShares.trace(shareCode).then(function (data) {
      if (data && data.sharer) {
        var sharer = data.sharer
        if (sharer.avatar_url) {
          sharer.avatar_url = apiModule.resolveAvatarUrl(sharer.avatar_url)
        }
        self.setData({ shareInfo: Object.assign({}, data, { sharer: sharer }) })
      }
    }).catch(function () {
      // 静默失败，不影响用户体验
    })
  ...
  doLogin: function () {
    ...
    if (d.inviteCode) {
      regData.invite_code = d.inviteCode
    }
    if (d.shareCode) {
      regData.share_code = d.shareCode
    }

    var p = d.isRegister
      ? api.auth.register(regData)
    ...
    p.then(function (data) {
      ...
      return api.auth.me().then(function (me) {
        ...
        return api.users.detail(me.id).then(function (detail) {
          if (d.isRegister) {
            profileGuide.setShowGuideAfterRegister()
          } else {
