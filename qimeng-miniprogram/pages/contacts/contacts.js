...
var refreshHint = require('../../utils/refresh-hint')
...
Page({
  data: {
    ...
    modal: null,
    guideStep: 0,
    guideSteps: profileGuide.GUIDE_STEPS,
    showProfileRestrict: false,
    drawerOpen: false,
    selectedMember: null,
  ...
  onShow: function () {
    ...
    this._openPendingOnLoad = !!wx.getStorageSync('qm_open_pending')
    if (this._openPendingOnLoad) {
      wx.removeStorageSync('qm_open_pending')
    }
    var app = getApp()
    if (app.globalData && app.globalData.contactsNeedRefresh) {
      app.globalData.contactsNeedRefresh = false
    }
  ...
  _maybeShowProfileGuide: function () {
    if (this._openPendingOnLoad) return
    if (this.data.modal === 'pending') return
    ...
    this.setData({ modal: 'guide', guideStep: 0 })
  },

  skipProfileGuide: function () {
    this.closeProfileGuide()
  },

  nextGuideStep: function () {
    var step = this.data.guideStep
    var total = (this.data.guideSteps || []).length
    if (step >= total - 1) {
      this.goProfileGuideEdit()
      return
    }
  ...
  closeProfileGuide: function () {
    this.setData({ modal: null, guideStep: 0 })
  },

  goProfileGuideEdit: function () {
    this.setData({ modal: null, guideStep: 0 })
    wx.navigateTo({ url: '/pages/edit-profile/edit-profile?from=guide' })
  },
  ...
  _load: function () {
    ...
    this.setData({ MY: { name: user.name || '', inviteCode: '', invitedCount: 0 }, loading: true, loadError: '' })

    function afterMe(me) {
      app.globalData.user = Object.assign({}, app.globalData.user, me)
      self.setData({ MY: { name: me.name || '', inviteCode: me.invite_code || '', invitedCount: me.invited_count || 0 } })
      if (me.id) {
        return api.users.detail(me.id).then(function (detail) {
          var card = detail && detail.card
          var incomplete = profileGuide.syncProfileHint(me, card)
          profileGuide.maybeSetShowGuide(me, card)
          self.setData({
            showProfileRestrict: incomplete
          })
          tabUtil.setTab(self, 0, {
            hasPending: (self.data.pendingRequests || []).length > 0,
            hasProfileIncomplete: incomplete
          })
        }).catch(function () {
          var incomplete = profileGuide.syncProfileHint(me, null)
          profileGuide.maybeSetShowGuide(me, null)
          self.setData({ showProfileRestrict: incomplete })
          tabUtil.setTab(self, 0, {
            hasPending: (self.data.pendingRequests || []).length > 0,
            hasProfileIncomplete: incomplete
          })
    ...
    var listP = Promise.all([
      ...
      tabUtil.setTab(self, 0, {
        ...
        hasProfileIncomplete: !!getApp().globalData.profileIncomplete
      })

      if (self._openPendingOnLoad) {
        self._openPendingOnLoad = false
        if (pending.length > 0) {
  ...
  onShareAppMessage: function () {
    if (this.data.modal === 'invite' || this.data._lastShareScene === 'invite') {
      var MY = this.data.MY || {}
      var code = this.data._inviteCode || MY.inviteCode || ''
      return {
        title: (MY.name || '好友') + ' 邀请你加入企盟',
        path: code ? '/pages/login/login?ref=' + code : '/pages/login/login'
      }
    }
  ...
  showInviteModal: function () {
    ...
    this.setData({
      modal: 'invite',
      _lastShareScene: 'invite',
      _inviteCode: MY.inviteCode || ''
    })
  },
  ...
  acceptRequest: function (e) {
    ...
    api.friends.accept(reqId).then(function () {
      self.setData({ modal: null })
      refreshHint.markContactsInboxDirty()
      self._load().then(function () {
        self._refreshInboxUnread()
      })
      wx.showToast({ title: '已成为好友', icon: 'success' })
    }).catch(function () {
      self._load()
    })
  },
  ...
  openMatchTask: function (e) {
    var id = parseInt(e.currentTarget.dataset.id, 10)
    if (!id) return
    getApp().globalData.tasksMainSub = 0
  ...
  openMatchActivity: function (e) {
    var id = parseInt(e.currentTarget.dataset.id, 10)
    if (!id) return
    getApp().globalData.tasksMainSub = 1
