...
from ..models.profit import StationProfitSettlement
...
def _serialize_booking_item(b: Booking, space: Space | None, now: datetime) -> dict:
    return {
        "id": b.id,
        "space_id": b.space_id,
        ...
        "audit_status": getattr(b, "audit_status", None) or "pending",
        "purpose": getattr(b, "purpose", None) or "",
        "participant_count": getattr(b, "participant_count", None) or 1,
        "note": getattr(b, "note", None) or "",
        "reject_reason": getattr(b, "reject_reason", None) or "",
...
def v1_my_bookings(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for b in rows:
        ...
        items.append(_serialize_booking_item(b, space, now))
...
def create_booking(payload: BookingCreate,
    ...
    audit_status = "approved" if is_free else "pending"
    pay_status = "paid" if is_free else "pending"
    b = Booking(
        space_id=s.id,
        user_id=user.id,
        ...
        participant_count=max(1, int(payload.participant_count or 1)),
    ...
    }, msg="预约已提交，请等待审核" if audit_status == "pending" else "预约成功")
...
def station_detail(station_id: int, db: Session = Depends(get_db)):
    ...
    op = db.query(User).filter(User.id == s.operator_id).first() if s.operator_id else None
    settlements = (
        db.query(StationProfitSettlement)
        .filter(StationProfitSettlement.station_id == station_id)
        .order_by(StationProfitSettlement.period.desc())
        .limit(6)
        .all()
    )
    return ok({
        ...
        "settlements": [{
            ...
            "net": st.net_profit,
            "distributed": st.distributed_amount,
