...
def _activity_brief(db: Session, a: Activity, signups: Optional[int] = None) -> dict:
    if signups is None:
...
def my_activities(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    posted = (
        db.query(Activity)
        .filter(Activity.host_id == user.id)
        .order_by(Activity.id.desc())
        .all()
    )
    joined_rows = (
        db.query(ActivityCheckin)
        .filter(ActivityCheckin.user_id == user.id)
        .order_by(ActivityCheckin.id.desc())
        .all()
    )
    joined = []
    for ck in joined_rows:
        a = db.query(Activity).filter(Activity.id == ck.activity_id).first()
        if not a:
            continue
        item = _activity_brief(db, a)
        item["signed_up_at"] = ck.signed_up_at
        item["checked_in_at"] = ck.checked_in_at
        joined.append(item)
    return ok({
        "posted": [_activity_brief(db, a) for a in posted],
        "joined": joined,
    })
...
def update_activity(
    ...
    """发起人编辑活动（已取消/已结束不可改）"""
    a = db.query(Activity).filter(Activity.id == activity_id, Activity.host_id == user.id).first()
    if not a:
        raise HTTPException(403, "无权限，仅发起人可编辑")
    if a.status in ("cancelled", "finished"):
        raise HTTPException(400, "活动已取消或已结束，无法编辑")
    ...
    if "cover_url" in data:
        a.cover_url = data["cover_url"] or ""
    if "location" in data:
        a.location = data["location"] or ""
    if "capacity" in data:
        cap = data["capacity"]
        if cap is not None and cap < 1:
            raise HTTPException(400, "人数上限至少为 1")
        signups = db.query(ActivityCheckin).filter(ActivityCheckin.activity_id == activity_id).count()
        if cap is not None and cap < signups:
            raise HTTPException(400, f"人数上限不能低于已报名人数（{signups}）")
        a.capacity = cap
    if "start_at" in data:
        a.start_at = data["start_at"]
    if "end_at" in data:
        a.end_at = data["end_at"]
    if "signup_deadline" in data:
        a.signup_deadline = data["signup_deadline"]
...
@router.patch("/{activity_id}/cancel")
def cancel_activity(
    activity_id: int,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
    ...
    a = db.query(Activity).filter(Activity.id == activity_id, Activity.host_id == user.id).first()
    if not a:
        raise HTTPException(403, "无权限，仅发起人可取消")
    if a.status in ("cancelled", "finished"):
        raise HTTPException(400, "活动已取消或已结束")
    a.status = "cancelled"
    db.commit()
    return ok(msg="活动已取消")
...
def signup(activity_id: int, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    from ..core.profit_share import grant_activity_referral_profit
    ...
    grant_contribution_once(
        ...
        "activity_signup",
        10,
        related_entity=f"activity_{activity_id}",
        note="活动报名",
    )
    grant_activity_referral_profit(db, user, activity_id)
    db.commit()
    return ok(msg="报名成功")
...
def _host_signup_attendees(db: Session, activity_id: int):
    ...
    for ck in rows:
        ...
        if not u:
            continue
        signed_in = bool(ck.signed_in or ck.checked_in_at)
        items.append({
            "user_id": u.id,
            "name": u.name or "",
            ...
            "signed_in": signed_in,
            "signed_in_at": (
                ck.signed_in_at.isoformat() if ck.signed_in_at
                else (ck.checked_in_at.isoformat() if ck.checked_in_at else None)
            ),
        })
    return ok({
        "items": items,
        "total": len(items),
        "activity_title": a.title if a else "",
        "activity_status": a.status if a else "",
    })
