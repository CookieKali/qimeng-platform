...
from ..schemas.profile import ProfileUpdate, ProfileSaveAll
from .profile_sync import (
    apply_profile_update,
    get_or_create_card,
    maybe_award_profile_contribution,
...
def _v1_save_profile(
    ...
    data = payload.model_dump(exclude_unset=True)
    apply_profile_update(db, user, data)
    maybe_award_profile_contribution(db, user)
    db.commit()
    return ok(msg="名片已更新")
...
@v1_router.post("/save-all")
def v1_save_all_profile(
    payload: ProfileSaveAll,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """一次保存用户基本信息 + 名片资料（小程序编辑资料页）"""
    data = payload.model_dump(exclude_unset=True)
    if "name" in data:
        ...
        if name:
            user.name = name
    if "email" in data:
        user.email = (data.pop("email") or "").strip()
    apply_profile_update(db, user, data)
    maybe_award_profile_contribution(db, user)
    db.commit()
    db.refresh(user)
    mp, card = _load_profile(db, user)
    return ok(member_profile_response(mp, card), msg="资料已保存")
