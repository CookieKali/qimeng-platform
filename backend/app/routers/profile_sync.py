...
def is_profile_complete(user: User, card: Card) -> bool:
    """与小程序 profileGuide 核心字段对齐，用于贡献积分「首次完善资料」"""
    name_ok = bool((user.name or "").strip())
    job_ok = bool((card.job_title or "").strip())
    region_ok = bool((card.region or "").strip())
    industry_ok = bool((card.industry or "").strip())
    phone_ok = bool((user.phone or "").strip())
    return name_ok and job_ok and phone_ok and (region_ok or industry_ok)


def maybe_award_profile_contribution(db: Session, user: User) -> None:
    from ..core.contribution import grant_contribution_once

    card = get_or_create_card(db, user)
    if not is_profile_complete(user, card):
        return
