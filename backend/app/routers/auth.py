...
from ..schemas.auth import RegisterIn, LoginIn, ProfileUpdate, WxBindIn
from ..core.wechat import code_to_openid, wechat_login_configured
...
def register(payload: RegisterIn, db: Session = Depends(get_db)):
    ...
    user = User(
        ...
        name=payload.name or f"用户{phone[-4:]}",
...
@router.post("/wx-bind")
async def wx_bind(payload: WxBindIn, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """绑定微信 openid（支付前必须）；需配置 WECHAT_APP_ID / WECHAT_APP_SECRET"""
    if not wechat_login_configured():
        raise HTTPException(503, "微信登录未配置")
    try:
        session = await code_to_openid(payload.code.strip())
    except ValueError as e:
        raise HTTPException(400, str(e)) from e
    openid = session["openid"]
    other = db.query(User).filter(User.wx_openid == openid, User.id != user.id).first()
    if other:
        raise HTTPException(400, "该微信已绑定其他账号")
    user.wx_openid = openid
    db.commit()
    return ok({"bound": True}, msg="微信已绑定")
