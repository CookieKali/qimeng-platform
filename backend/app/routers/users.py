...
from ..core.avatars import save_user_avatar, delete_user_avatar
...
@v1_router.delete("/me/avatar")
def delete_my_avatar(
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """清除当前用户上传的头像（恢复为默认字母头像）"""
    delete_user_avatar(user.id)
    user.avatar_url = ""
    db.commit()
    return ok({"avatar_url": ""}, msg="头像已清除")
