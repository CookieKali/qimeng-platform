...
def _friend_request_status(db: Session, m: InboxMessage) -> Optional[str]:
    """pending / accepted / done / gone — 仅 friend_request 类型"""
    if m.type != "friend_request":
        return None
    if not m.related_id:
        return "gone"
    r = db.query(FriendRequest).filter(FriendRequest.id == m.related_id).first()
    if not r:
        return "gone"
    if r.status == "pending" and r.to_user_id == m.to_user_id:
        return "pending"
...
def _sync_stale_friend_request_inbox(db: Session, user_id: int) -> None:
    """申请已处理或双方已是好友时，自动将对应通知标为已读，避免红点常驻"""
    rows = db.query(InboxMessage).filter(
    ...
    changed = False
    for m in rows:
        st = _friend_request_status(db, m)
...
def _serialize_message(db: Session, m: InboxMessage) -> dict:
    ...
    if m.type == "friend_request":
        payload["request_status"] = _friend_request_status(db, m)
    return payload
...
def list_messages(
    ...
    _sync_stale_friend_request_inbox(db, user.id)
    page = max(1, page)
